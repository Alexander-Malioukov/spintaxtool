...
//					console.log(aReadWords);
	...
	function createParagraphZone(noParagraph, paragraphTitle, paragraphContent) {
			...
			$paragraphZone = $('' +
				...
				'<div class="paragraph-container" data-title="' + paragraphTitle + '">' +
					...
					'<div class="grid collapse-with-following">' +
						...
						'</div>' +
						'<div class="grid random-opt-area align-right" id="random-opt-area-'+noParagraph+'">'+' <input type="radio" name="cnt_par[' + noParagraph + '][h2op]" value="prmnent" > ' + MySpeech.get("form.select.permanenthtag") + ' &nbsp;&nbsp;  <input type="radio" name="cnt_par[' + noParagraph + '][h2op]" value="rndm" > ' + MySpeech.get("form.select.randmhtag") + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; '+MySpeech.get("form.select.min_ele_keep")+''+
		...
		var countElement	   =  elementCountFlatspin(readableContent);
		//var countElement       =  Flatspin.countReadbleElement(readableContent);
		
		$("#detected-elements-"+ noParagraph).html(countElement);
		$paragraphZone.find('input[name="cnt_par['+noParagraph+'][rand_from]"]').val(countElement);
		$paragraphZone.find('input[name="cnt_par['+noParagraph+'][rand_to]"]').val(countElement);
		$paragraphZone.find('input[name="cnt_par['+noParagraph+'][rand_fr]"]').val(1);
		$paragraphZone.find('input[name="cnt_par['+noParagraph+'][rand_t]"]').val(countElement);
    ...
    function saveTmpData( $content, $id ) {
    	//console.log($id);
        var zip = new jszip(),
...
//		result.qualityIndex = ( !result.totalHoles ? 0 : parseFloat((( result.totalVariations / result.totalHoles ) / ( _this.REFERENCE_RATE.VARIATION / _this.REFERENCE_RATE.PERFORATION )).toFixed(1)) );
	...
	this.analyzeArticle = function() {
		...
		$('#spintax-article-container').slideUp(400, function() {
            ...
            //console.log( aParagraphsContents );
    		$.each(aParagraphsContents, function (i,v) {
    ...
    //		ParagraphNavigator.fillAndShow($('textarea[id^=spintax-paragraph-]').length);
		...
		$('#submitButton').html(MySpeech.get("form.button.create_article")).off('click').on('click', getReadbleContent);
...
if(first_box>second_box){	first_box=second_box;	}
