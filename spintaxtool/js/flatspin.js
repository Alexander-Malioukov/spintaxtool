function Flatspin() {
	...
	function generateVariations() {
		var aSpintaxSentences = $("#paragraph-spintax").val().replace(/\|{2,}/g, '\|').split("\n"),
...
//			console.log(error.message, error.start, countReturn);
	...
	this.generateFlatspin = function() {

		// Can we process ?

		// ----------------
		$("#paragraph-spintax").val( $("#paragraph-spintax").val().trim() );

		

		if ( !$("#paragraph-spintax").val() ) {

			MyUtils.displayInfoMessage('warning', MySpeech.get("message.warning.please_enter_spintax_paragraph"));

			$("#paragraph-spintax").focus();
...
/*		if ( $("#paragraph-spintax").val().split('\n\n').length > 1 ) {

			MyUtils.displayInfoMessage('warning', MySpeech.get("message.warning.please_enter_only_one_paragraph"));
		...
		$('#main-zone').slideUp(400, function() {

			// Flatspin generation

			// -------------------

			ButtonHandler.showLoader( $('#submitButton') );
			$("#paragraph-variations").val( generateVariations() );

			

			// Spellcheck variations paragraph

			// -------------------------------

//			spellcheckVariations( $("#paragraph-variations").val() ); // JSI

//			MyUtils.displayInfoMessage('info', MySpeech.get("message.info.spellchecking_complete"));

			$('#main-zone').slideDown(400);

			ButtonHandler.hideLoader( $('#submitButton') );
