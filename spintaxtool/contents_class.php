...
class contentsClass
{
    ...
    public function generateRandomParagraph($bodyElementArr, $p_from, $p_to, $other = '')
    {
        ...
        if (!empty($topParagrapArr))
        {
            $finalParagrapArr = array_merge($topParagrapArr, $new_swappablePar);
        }
        else
        {
            $finalParagrapArr = $new_swappablePar;
        }
        if (!empty($bottomParagrapArr))
        {
            $finalParagrapArr = array_merge($finalParagrapArr, $bottomParagrapArr);
        }
    ...
    public function add_Ptag($splited_file_data)
    {
        ...
        foreach ($splited_file_data as $para)
        {
            ...
            if (!empty($para) && $match == 0)
            {
                ...
                if ($cnt < $total_para - 1) $newDataWithTag[$cnt] .= "\n\n";
            }
