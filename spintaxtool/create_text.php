...
{
    echo $obj->render_popup('Sorry, Data is empty (1). Please try again or contact to admin');
    exit();
}
else
{
    ...
    if (!$formData['cnt_par'])
    {
        echo $obj->render_popup('Sorry, Data is empty (2). Please try again or contact to admin');
        exit();
    }
}
...
for ($j = 0;$j <= $no_article;$j++)
{
    ...
    foreach ($textData as $tD)
    {
        ...
        else array_push($textData_arr, $tD);
    }
    ...
    if (!function_exists('ensure_html_tag_closure'))
    {
        function ensure_html_tag_closure($html_chunk)
        {
            ...
            if (!empty($start_tag) && isset($start_tag[0]) && !empty($start_tag[0]))
            {
                ...
                if (empty($end_tag) || !isset($end_tag[0]) || empty($end_tag[0]))
                {
    ...
    echo '<hr /><textarea>' . print_r($textData, true) . '</textarea>';
...
echo $obj->render_popup($mess, $downloadLink);
exit();
?>
